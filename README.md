...
## **功能模块**
 ...
 ### **1.首页**
    对于新用户来说，首先引导页简单介绍软件功能，新用户可以不用注册便可简单体验学习内容，针对汉语掌握程度不同的用户我们设置了<br>
    不同等级，可以提供与之水平匹配的学习内容。我们的软件的主要特色就是纠正用户的汉语发音，再此基础上加入词汇书、句法书供用户<br>
    学习。页面顶栏搜索框可用于搜索单词句子，拍照识别，也可上传文件，进行文件翻译。对于如何确定录音结束等问题，软件中设置了<br>
    初试时间自动结束，通过检测分贝来判断语音是否输入结束，另外增加“我已读完”按钮。在学习页面中，我们设置了语音识别、评判<br>
    等级等功能，根据声调、短句、发音等指标对用户语音输入进行打分，并根据结果日出个性化建议。  
