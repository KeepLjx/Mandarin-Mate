		...
		<tm-sheet :height="60" :width="_width" :round="24" unit="px" _class="flex-center flex " parenClass="relative"
			...
			<tm-badge :fontSize="20" :color="c_font_style.dotColor" :eventPenetrationEnabled="true" :dot="props.dot"
				...
				<view :class="[_active ? 'anifun' : '']" class="flex flex-col flex-col-center-center"
					...
					<slot>
						<tm-icon :customicon="props.customicon" v-if="!_load && !props.customIcon" _style="line-height: 0px;"
							:color="_color" :font-size="c_font_style.iconSize"
							:name="_active ? c_font_style.icon : c_font_style.unicon || c_font_style.icon"></tm-icon>
					</slot>
					<tm-icon v-if="_load && !props.customIcon" spin _style="line-height: 0px;" :color="_color"
						:font-size="c_font_style.iconSize" name="tmicon-shuaxin"></tm-icon>
					<image v-if="props.customIcon" src="../../../static/icons/shiyinNot.svg" mode="scaleToFill"
